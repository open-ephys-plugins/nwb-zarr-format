	...
	const Array<const SpikeChannel*>& electrodeArray)
{
	...
	ScopedPointer<ZarrDataset> elSet = createDataSet(BaseDataType::I32, 1, 1, "general/extracellular_ephys/electrodes/id");
	...
	ScopedPointer<ZarrDataset> groupNamesDataset = createDataSet(BaseDataType::STR(250), 0, 1, "general/extracellular_ephys/electrodes/group_name");
	...
	ScopedPointer<ZarrDataset> locationsDataset = createDataSet(BaseDataType::STR(250), 0, 1, "general/extracellular_ephys/electrodes/location");
...
void NWBFile::writeChannelConversions(ecephys::ElectricalSeries* electricalSeries)
{
  ...
  bool NWBFile::createTimeSeriesBase(TimeSeries* timeSeries)
 {
  ...
  ZarrDataset* NWBFile::createTimestampDataSet(String path, int chunk_size, float interval)
  {
	  
	  ZarrDataset* tsSet = createDataSet(BaseDataType::F64, 0, chunk_size, path);
   ...
   ZarrDataset* NWBFile::createSampleNumberDataSet(String path, int chunk_size)
  {
	  ZarrDataset* tsSet = createDataSet(BaseDataType::I64, 0, chunk_size, path);
	  if (!tsSet)
...
ZarrDataset *NWBFile::createChannelConversionDataSet(String path, String description, int chunk_size)
{
   ZarrDataset *elSet = createDataSet(BaseDataType::F32, 1, chunk_size, path);
...
ZarrDataset *NWBFile::createElectrodeDataSet(String path, String description, int chunk_size)
{
	ZarrDataset *elSet = createDataSet(BaseDataType::I32, 1, chunk_size, path);
	if (!elSet)
		std::cerr << "Error creating electrode dataset in " << path << std::endl;
	else
	{
  ...
  bool NWBFile::createEventMetadataSets(String basePath, TimeSeries* timeSeries, const MetadataEventObject* info)
  {
	  ...
	  for (int i = 0; i < nMetadata; i++)
	  {
		  ...
		  String fullPath = basePath + "/" + fieldName;
		  ZarrDataset* dSet = createDataSet(type, 0, length, EVENT_CHUNK_SIZE, fullPath);
  ...
  {
	  ScopedPointer<ZarrDataset> dSet;
