...
namespace NWBRecording
{
	...
	public:
		...
		ScopedPointer<ZarrDataset> baseDataSet;
        
        /** Holds the timestamps (in seconds) for each sample */
		ScopedPointer<ZarrDataset> timestampDataSet;
        
        /** Holds the sample number for each sample (relative to the start of acquisition) */
		ScopedPointer<ZarrDataset> sampleNumberDataSet;
        
        /** Holds metadata for this time series */
        OwnedArray<ZarrDataset> metaDataSet;
    ...
    public:
        ...
        /** Holds the sample number for each sample (relative to the start of acquisition) */
        ScopedPointer<ZarrDataset> channelConversionDataSet;
        
        /** Holds the DynamicTableRegion index of each electrode  */
        ScopedPointer<ZarrDataset> electrodeDataSet;
    ...
    public:
        ...
        ScopedPointer<ZarrDataset> ttlWordDataSet;
    ...
    /**
        ...
        Represents an NWB 2.0 File (a specific type of Zarr file)
            
     */
	class NWBFile : public ZarrFileBase
	{
	public:
	...
	private:
		...
		void createBinaryDataSet(String path, String name, ZarrFileBase::BaseDataType type, int length, void* data);
        
        /** Returns the HDF5 data type for a given event channel type */
		static ZarrFileBase::BaseDataType getEventH5Type(EventChannel::Type type, int length = 1);
		
        /** Returns the HDF5 data type for a given metadata type*/
        static ZarrFileBase::BaseDataType getMetadataH5Type(MetadataDescriptor::MetadataType type, int length = 1);
		...
		ZarrDataset* createTimestampDataSet(String basePath, int chunk_size, float interval);
        
        /** Creates a dataset of sample numbers */
		ZarrDataset* createSampleNumberDataSet(String basePath, int chunk_size);
        
        /** Creates a dataset for electrode indices */
		ZarrDataset* createElectrodeDataSet(String basePath, String description, int chunk_size);
        
        /** Creates a dataset for electrode indices */
        ZarrDataset* createChannelConversionDataSet(String basePath, String description, int chunk_size);
